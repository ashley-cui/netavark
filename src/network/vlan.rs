...
impl driver::NetworkDriver for Vlan<'_> {
    ...
    fn validate(&mut self) -> NetavarkResult<()> {
        ...
        let opts = parse_vlan_opts(&self.info.network.options)?;
...
pub fn parse_vlan_opts(opts: &Option<HashMap<String, String>>, strict: bool) -> NetavarkResult<VlanOptions> {
    if strict {
        if let Some(invalid_key) = opts.as_ref().and_then(|m| m.keys().find(|k| !VALID_VLAN_OPTS.contains(&k.as_str()))) {
            return Err(NetavarkError::msg(format!("unsupported vlan network option: {}", invalid_key)));
        }
    }
