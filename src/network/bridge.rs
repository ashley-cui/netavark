...
impl driver::NetworkDriver for Bridge<'_> {
    ...
    fn validate(&mut self) -> NetavarkResult<()> {
        ...
        let opts = parse_bridge_opts(&self.info.network.options)?;
...
pub fn parse_bridge_opts(opts: &Option<HashMap<String, String>>, strict: bool) -> NetavarkResult<BridgeOptions> {
    if strict {
        if let Some(invalid_key) = opts.as_ref().and_then(|m| m.keys().find(|k| !VALID_BRIDGE_OPTS.contains(&k.as_str()))) {
            return Err(NetavarkError::msg(format!("unsupported bridge network option: {}", invalid_key)));
        }
    }
    ...
    if vlan.is_some() {
        if vlan.unwrap() > 4094 {
            return Err(NetavarkError::msg(format!("vlan must be between 0 and 4094")));
        }
    }
    Ok(BridgeOptions {
        mode,
