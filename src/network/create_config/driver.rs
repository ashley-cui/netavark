...
use crate::network::types::{Network, Used, CreateOpts};
use crate::{
    error::{JsonError, NetavarkError, NetavarkResult},
    wrap,
};
use crate::network::create_config::subnet::{get_free_ipv4_network_subnet, get_free_ipv6_network_subnet};
...
pub fn setup_bridge_options(network: &mut Network) -> NetavarkResult<(bool, bool)> {
    ...
    for key in keys {
        ...
        match key.as_str() {
            ...
            _ => return Err(NetavarkError::msg(format!("unsupported bridge network option {}", key))),
        }
    }
...
pub fn create_bridge(network: &mut Network, used: &Used, check_used: bool, check_bridge_conflict: bool, opts: CreateOpts) -> NetavarkResult<()> {
    match &network.network_interface {
        Some(interface) => {
            if check_bridge_conflict {
            if used.interfaces.contains(interface) {
                return Err(NetavarkError::msg(format!("bridge name {} already in use", interface)));
            }
            }
            let interface_regex = Regex::new(r"^[a-zA-Z0-9][a-zA-Z0-9_.-]*$").unwrap();
            if !interface_regex.is_match(interface) {
                return Err(NetavarkError::msg(format!("bridge name {} is invalid", interface)));
            }
        }
        None => {
    ...
    if should_use_hostlocal {
        let has_subnets = network
            .subnets
            .as_ref()
            .map_or(false, |subnets| !subnets.is_empty());
        if !has_subnets {
            let free_subnet = get_free_ipv4_network_subnet(&used.subnets, &opts.subnet_pools, check_used)?;
        ...
        network.ipam_options.get_or_insert_with(HashMap::new).insert("driver".to_string(), constants::IPAM_HOSTLOCAL.to_string());
    }
    ...
    match ipam_driver {
        "" => {
            ...
            if subnets_len == 0 {
                ...
                if !is_macvlan {
                    return Err(NetavarkError::msg("ipam driver dhcp is not supported with ipvlan"));
                }
            } else {
        ...
        d if d == constants::IPAM_DHCP => {
            ...
            let subnets_len = network.subnets.as_ref().map_or(0, |s| s.len());
            if subnets_len > 0 {
                return Err(NetavarkError::msg("ipam driver dhcp set but subnets are set"));
            }
        }
        _ => {}
    }
        ...
        for (key, value) in options_clone {
            match key.as_str() {
                constants::OPTION_MODE => {
                    if is_macvlan {
                        if !constants::VALID_MACVLAN_MODES.contains(&value.as_str()) {
                            return Err(NetavarkError::msg(format!("unknown macvlan mode {:?}", value)));
                        }
                    } else {
                        if !constants::VALID_IPVLAN_MODES.contains(&value.as_str()) {
                            return Err(NetavarkError::msg(format!("unknown ipvlan mode {:?}", value)));
                        }
                    }
                }
                constants::OPTION_METRIC => {
                ...
                constants::OPTION_BCLIM => {
                    if is_macvlan {
                        value.parse::<i32>()
                            .map_err(|e| NetavarkError::msg(format!("failed to parse {:?} option: {}", key, e)))?;
                        // do not fallthrough for macvlan
                    } else {
                        // bclim is only valid for macvlan not ipvlan so fallthrough to error case
                        return Err(NetavarkError::msg(format!("unsupported {} network option {}", driver, key)));
                    }
                }
                _ => {
                    return Err(NetavarkError::msg(format!("unsupported {} network option {}", driver, key)));
...
fn get_free_device_name(
    ...
    Err(NetavarkError::msg("Could not find a free device name after 1,000,000 attempts"))
}
...
fn parse_isolate(isolate: &str) -> NetavarkResult<String> {
    match isolate {
        ...
        _ => Err(NetavarkError::msg(format!("failed to parse isolate option {}", isolate))),
    }
}
...
fn parse_no_default_route(ndr: &str) -> NetavarkResult<String> {
    match ndr{
        ...
        _ => Err(NetavarkError::msg(format!("invalid no_default_route value {}", ndr))),
    }
}
