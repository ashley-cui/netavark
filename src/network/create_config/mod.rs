...
pub fn new_network(
    ...
    match network.driver.as_str() {
        constants::DRIVER_BRIDGE => {
            ...
            create_bridge(&mut network, &create.used, check_used, check_bridge_conflict, create.create_opts)?;
        },
    ...
    if network
        ...
        .unwrap_or(false)
    {
        log::debug!("dns disabled for network {:?} because ipam driver is set to none", network.name);
        network.dns_enabled = false;
    }
    if network.network_dns_servers.is_some() && !network.dns_enabled {
        return Err(NetavarkError::msg("cannot set NetworkDNSServers if DNS is not enabled for the network"));
    }
