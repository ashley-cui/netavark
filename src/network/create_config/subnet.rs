...
pub fn network_intersects_with_networks(network: &IpNet, used_networks: &[IpNet]) -> bool {
    used_networks.iter().any(|used_net| {
        match (network, used_net) {
...
pub fn get_free_ipv4_network_subnet(
    ...
    for pool in subnet_pools {
        ...
        let pool_base_network_addr: Ipv4Addr = pool_v4.network().into();
        let mut network = match Ipv4Net::new(pool_base_network_addr, pool.size as u8) {
            Ok(net) => net,
            Err(e) => {
                last_error = Some(NetavarkError::msg(format!("Failed to create network from pool: {}", e)));
                continue;
            }
        };
        
        loop {
            let network_addr: Ipv4Addr = network.network().into();
            if !pool_v4.contains(&network_addr) {
                break;
            }
            
            // If check_used is true, only return if the network doesn't intersect with used networks
            let found = !(check_used && network_intersects_with_networks(&IpNet::V4(network), used_networks));
            if found {
                return Ok(Subnet {
                    gateway: None,
                    lease_range: None,
    ...
    if let Some(err) = last_error {
        ...
        Err(NetavarkError::msg("could not find free subnet from subnet pools"))
    }
}
...
pub fn get_free_ipv6_network_subnet(
    used_networks: &[IpNet],
) -> NetavarkResult<Subnet> {
    ...
    for _ in 0..MAX_ATTEMPTS {
        ...
        let network = match get_random_ipv6_subnet() {
            Ok(net) => net,
            Err(e) => return Err(e),
        };
