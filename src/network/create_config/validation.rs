...
pub fn validate_name_id(name: &str, id: &str, existing_networks: &HashMap<String, String>) -> NetavarkResult<()> {
    if name.is_empty() {
        return Err(NetavarkError::msg("Network name must be supplied"));
    }
    let name_regex = Regex::new(r"^[a-zA-Z0-9][a-zA-Z0-9_.-]*$").unwrap();
    if !name_regex.is_match(name) {
        return Err(NetavarkError::msg("Invalid characters in network name: must match [a-zA-Z0-9][a-zA-Z0-9_.-]"));
    }
    if existing_networks.contains_key(name) {
        return Err(NetavarkError::msg(format!("network already exists {}", name)));
    }
    ...
    if !hex_regex.is_match(id) {
        return Err(NetavarkError::msg("Network id must be a hexadecimal string"));
    }
    Ok(())
}
...
pub fn validate_interface_name(if_name: &str) -> NetavarkResult<()> {
    if if_name.chars().count() > constants::MAX_INTERFACE_NAME_LEN {
        return Err(NetavarkError::msg(format!(
            ...
            constants::MAX_INTERFACE_NAME_LEN, if_name
        )));
    }
...
pub fn validate_ipam_driver(ipam_opts: &Option<HashMap<String, String>>, subnets: &Option<Vec<Subnet>>) -> NetavarkResult<()> {
    ...
    match ipam_driver {
        ...
        Some(driver) if driver == constants::IPAM_NONE => {
            ...
            if subnetlen > 0 {
                return Err(NetavarkError::msg("None ipam driver is set but subnets are given"));
            }
            Ok(())
...
pub fn validate_subnets(network: &mut Network, add_gateway: bool, check_used: bool, used_networks: &[IpNet]) -> NetavarkResult<()> {
...
fn validate_lease_range(lease_range: &crate::network::types::LeaseRange, subnet: &IpNet) -> NetavarkResult<()> {
...
pub fn validate_subnet(s: &mut Subnet, add_gateway: bool, check_used: bool, used_networks: &[IpNet]) -> NetavarkResult<()> {
