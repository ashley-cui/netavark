...
pub(crate) fn handle_plugin_error(code: i32, buffer: &[u8], plugin_name: Option<&OsStr>) -> NetavarkError {
    ...
    fn exec_plugin(&self, setup: bool, netns: &str) -> NetavarkResult<Option<types::StatusBlock>> {
        ...
        match result {
            ...
            PluginResult::Error { code, buffer } => {
                Err(handle_plugin_error(code, &buffer, None))
            }
            PluginResult::Killed => {
                Err(handle_plugin_killed(None))
            }
        }
    }
}
